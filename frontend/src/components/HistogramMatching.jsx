...
export const HistogramMatching = () => {
	...
	const [imageName, setImageName] = useState("histogram-matching-res.nii"); // Estado para almacenar el nombre de la imagen
    ...
    let getHistogram = async () => {
        if (input.HistogramMatchingH) {
            setInput({ ...input, HistogramMatchingH: false });
        }
        setTimeout(() => {
             setInput({ ...input, HistogramMatchingH: true });
         }, 1000);
	...
	return (
		<div className="flex flex-col items-center justify-center mt-9">
			<Navbar />
			<h1 className="text-lg font-bold text-black m-4">Histogram Matching </h1>
            ...
            {input.HistogramMatchingH && (
                <div className='flex flex-row'>

                    <img className='flex flex-col mx-2' src={"http://localhost:8000/image/histogram/"+imageName} alt="histogram HistogramMatching" />
